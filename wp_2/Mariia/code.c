...
{
    // declare a variable that will be holding controller code
    ...
    if (argc == 6)           // check max number of arguments
    {
        ...
        int engine_on, gear_pos, key_pos, brake1, brake2; // declare variables for storing user input numbers
        ...
        gear_pos = atoi(argv[2]);  // input at postion 2 corresponds to gear_pos
        key_pos = atoi(argv[3]);   // input at position 3 corresponds to key_pos
        ...
        if ((engine_on == 0 || engine_on == 1) && // valid range for engine_on: 0-1
            (gear_pos >= 0 && gear_pos <= 7) &&   // valid range for gear_pos: 0-7
            (key_pos >= 0 && key_pos <= 2) &&     // valid range for key_pos: 0-2
            ...
            (brake2 == 0 || brake2 == 1))         // valid range for brake2: 0-1
        {
            ...
            eCode |= gear_pos << 4;  // set bits 2-4 to the value of floor_pos. any number in the range of 0-7 will have binary representation of 3 bits
            eCode |= key_pos << 2;   // set bits 5-6 to the value of door_pos
